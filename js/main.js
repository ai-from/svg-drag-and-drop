...
function endDrag(event) {
  ...
  let rightSvg =
    svg.getBoundingClientRect().left + svg.getBoundingClientRect().width;
  let bottomSvg =
    svg.getBoundingClientRect().top + svg.getBoundingClientRect().height;
  let leftSvg = svg.getBoundingClientRect().left;

  let topSquare = square.getBoundingClientRect().top;
  let rightSquare =
    square.getBoundingClientRect().left + square.getBoundingClientRect().width;
  let bottomSquare =
    square.getBoundingClientRect().top + square.getBoundingClientRect().height;
  let leftSquare = square.getBoundingClientRect().left;

  let israspberry =
    topSvg >= topSquare &&
    rightSvg <= rightSquare - 150 &&
    bottomSvg <= bottomSquare - 150 &&
    leftSvg >= leftSquare;
  let isBlue =
    topSvg >= topSquare &&
    rightSvg <= rightSquare &&
    bottomSvg <= bottomSquare - 150 &&
    leftSvg >= leftSquare + 150;
  let isGreen =
    topSvg >= topSquare + 150 &&
    rightSvg <= rightSquare - 150 &&
    bottomSvg <= bottomSquare &&
    leftSvg >= leftSquare;
  let isPurple =
    topSvg >= topSquare + 150 &&
    rightSvg <= rightSquare &&
  ...
  if (israspberry && !blockStatus.raspberry && isTopCircle) {
    addCircle(1);
    blockStatus.raspberry = true;
...
function addCircle(num) {
  ...
  path1.setAttribute(
    "d",
    "M41 21.0003C41 32.046 32.0457 41.0003 21 41.0003C9.9543 41.0003 1 32.046 1 21.0003C7.5 21.0002 9.9543 21.0002 21 21.0002C32.0457 21.0002 36 21.0002 41 21.0003Z"
  );

  let path2 = document.createElementNS("http://www.w3.org/2000/svg", "path");
  path2.setAttribute(
    "d",
    "M1 21C1 9.95428 9.9543 0.999975 21 0.999975C32.0457 0.999975 41 9.95428 41 21C34.5 21 32.0457 21 21 21C9.9543 21 6 21 1 21Z"
  );

  if (num === 1) {
    g.setAttribute("transform", "translate(54.5, 54.5)");
    path1.setAttribute("fill", "#49C2F1");
    path2.setAttribute("fill", "#E30B5D");
  }
  if (num === 2) {
